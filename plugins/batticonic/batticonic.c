 ...
 *               2017 Vladimir Kamensky <info@mastersoft24.ru>
 *
...
//      Icon names
#define ICONS_BATTERY_CAUTION                   "battery-caution"
#define ICONS_BATTERY_CAUTION_CHARGING		"battery-caution-charging"

#define ICONS_BATTERY_EMPTY                     "battery-empty"
#define ICONS_BATTERY_EMPTY_CHARGING		"battery-empty-charging"

#define ICONS_BATTERY_FULL                      "battery-full"
#define ICONS_BATTERY_FULL_CHARGED		"battery-full-charged"
#define ICONS_BATTERY_FULL_CHARGING		"battery-full-charging"

#define ICONS_BATTERY_GOOD                      "battery-good"
#define ICONS_BATTERY_GOOD_CHARGING		"battery-good-charging"

#define ICONS_BATTERY_LOW                       "battery-low"
#define ICONS_BATTERY_LOW_CHARGING		"battery-low-charging"

#define ICONS_BATTERY_MISSING                   "battery-missing"


#define PERCENT_BATTERY_EMPTY 3
#define PERCENT_BATTERY_CAUTION 10
#define PERCENT_BATTERY_LOW 20
#define PERCENT_BATTERY_GOOD 40
#define PERCENT_BATTERY_FULL 99
...
static gchar* getBatteryIconName(int level, gboolean isCharging)
{
    gchar* bs;
    
    if(level >= PERCENT_BATTERY_FULL){
        
        if((level ==100)&&(isCharging)){
            bs=ICONS_BATTERY_FULL_CHARGED;
        }
        else{
            bs=ICONS_BATTERY_FULL_CHARGING;
        }
        
        if(!isCharging){
            bs=ICONS_BATTERY_FULL;
        }
        
        
    }
    
    if((level >= PERCENT_BATTERY_GOOD) && (level < PERCENT_BATTERY_FULL)){
        
        if(isCharging){
            bs=ICONS_BATTERY_GOOD_CHARGING;
        }
        else{
            bs=ICONS_BATTERY_GOOD;
        }
    }
    
    if((level >= PERCENT_BATTERY_LOW) && (level < PERCENT_BATTERY_GOOD)){
        
        if(isCharging){
            bs=ICONS_BATTERY_LOW_CHARGING;
        }
        else{
            bs=ICONS_BATTERY_LOW;
        }
        
    }    
    
    if((level >= PERCENT_BATTERY_EMPTY) && (level < PERCENT_BATTERY_LOW)){
        
        if(isCharging){
            bs=ICONS_BATTERY_CAUTION_CHARGING;
        }
        else{
            bs=ICONS_BATTERY_CAUTION;
        }        
        
    }       
    
    if(level < PERCENT_BATTERY_EMPTY){
        
        if(isCharging){
            bs=ICONS_BATTERY_EMPTY_CHARGING;
        }
        else{
            bs=ICONS_BATTERY_EMPTY;
        }        
...
void update_display(lx_battery *lx_b, gboolean repaint) {
    ...
    if( b == NULL )
    {
        ...
        set_statusicon_tooltips(lx_b->status_icon, _("No batteries found") );
        if (lx_b->hide_if_no_battery)
        {
           
            set_statusicon_visible(lx_b->status_icon,FALSE);
            repaint = FALSE;
            return;
        }
    ...
    iconName=getBatteryIconName(lx_b->b->percentage,isCharging);
    icon=iconName;
    ...
    if(lx_b->useSymbolic){ 
        iconName = g_strdup_printf(_("%s-symbolic"), icon);
    }
  
    
    set_statusicon_tooltips(lx_b->status_icon, tooltip);
    update_statusicon(lx_b->status_icon, iconName, iconName);
    set_statusicon_visible(lx_b->status_icon, TRUE);
...
static GtkWidget * constructor(LXPanel *panel, config_setting_t *settings)
{
    ...
    gtk_widget_set_has_window(p, FALSE);
            ...
            lx_b->mainw = panel_box_new(panel, FALSE, 1);
            gtk_widget_show_all(lx_b->mainw);
            gtk_container_add((GtkContainer*)p, lx_b->mainw);
    ...
    if (bat == NULL)
    {
        ...
        lx_b->status_icon = create_statusicon(lx_b->panel, lx_b->mainw,"battery-full-symbolic" , "tooltip", "battery-full-symbolic");
    ...
    if (config_setting_lookup_int(settings, "UseSymbolic", &tmp_int))
        lx_b->useSymbolic = (tmp_int != 0);   
...
//    config_group_set_int(b->settings, "Size", b->thickness);
    ...
    config_group_set_int(b->settings, "UseSymbolic", b->useSymbolic);
...
static GtkWidget *config(LXPanel *panel, GtkWidget *p) {
    ...
    return lxpanel_generic_config_dlg(_("New Battery Monitor"),
            panel, applyConfig, p,
            ...
            _("Alarm time (minutes left)"), &b->alarmTime, CONF_TYPE_INT,
             ...
             _("Use symbolic icon style"), &b->useSymbolic, CONF_TYPE_BOOL,
...
FM_DEFINE_MODULE(lxpanel_gtk, batticonic)

/* Plugin descriptor. */
LXPanelPluginInit fm_module_init_lxpanel_gtk = {
    .name        = N_("New Battery Monitor"),
    .description = N_("Display battery status using ACPI with iconic style"),
